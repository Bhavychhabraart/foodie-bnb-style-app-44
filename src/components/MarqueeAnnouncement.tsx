...
const MarqueeAnnouncement: React.FC<MarqueeProps> = ({ title, items }) => {
  return (
    <div className="relative bg-gradient-to-r from-airbnb-darkbrown via-airbnb-darkbrown/95 to-airbnb-darkbrown py-3 overflow-hidden border-y border-airbnb-gold/30 shadow-md shadow-black/10">
      ...
      <div className="absolute left-0 top-0 bottom-0 w-12 bg-gradient-to-r from-airbnb-darkbrown to-transparent z-[1]"></div>
          ...
          {[...Array(2)].map((_, dupeIndex) => (
            <React.Fragment key={dupeIndex}>
              ...
              {items.map((item, index) => (
                <span key={`${dupeIndex}-${index}`} className="flex items-center text-airbnb-beige">
